...
func (cq *crossQuotaPlugin) predicateFn(task *api.TaskInfo, node *api.NodeInfo) ([]*api.Status, error) {
	...
	if !ok {
		return predicateStatus, api.NewFitError(task, node, "node not tracked by crossquota plugin")
	}
	...
	for _, rn := range cq.quotaResourceNames {
		...
		if used+req > quota {
			return predicateStatus, api.NewFitError(task, node, fmt.Sprintf(
