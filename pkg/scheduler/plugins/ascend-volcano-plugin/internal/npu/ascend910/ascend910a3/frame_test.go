...
func TestBase910A3_CheckReqNPUEqualNodeNPU(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			base910A3:      newBase910A3(16, []TaskConfig{{TaskID: "task1", ReqNPUNum: 16}, {TaskID: "task2", ReqNPUNum: 16}}),
			expectedResult: nil, // Should pass, return nil
		},
		{
			name:           "02 Task with zero NPU and scheduler spec annotation - should pass",
			base910A3:      newBase910A3(16, []TaskConfig{{TaskID: "task1", Annotation: map[string]string{taskSpec: schedulerSpec}}}),
			expectedResult: nil, // Should pass, return nil
		},
		{
			name:           "03 Task with zero NPU and skip ascend plugin annotation - should pass",
			base910A3:      newBase910A3(16, []TaskConfig{{TaskID: "task1", Annotation: map[string]string{skipAscendPlugin: skipEnabled}}}),
			expectedResult: nil, // Should pass, return nil
		},
		{
			name:           "04 Empty tasks list - should pass",
			base910A3:      newBase910A3(16, []TaskConfig{}),
			expectedResult: nil, // Should pass, return nil
		},
	}
...
func TestBase910A3_CheckReqNPUEqualNodeNPU_ExpectError(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			base910A3:      newBase910A3(16, []TaskConfig{{TaskID: "task1"}}),
			expectedResult: &api.ValidateResult{Pass: false, Reason: JobCheckFailedReason, Message: "distributed job require npu 16, instead of 0"},
		},
		{
			name:           "02 Task with incorrect NPU number - should fail",
			base910A3:      newBase910A3(16, []TaskConfig{{TaskID: "task1", ReqNPUNum: 8}}),
			expectedResult: &api.ValidateResult{Pass: false, Reason: JobCheckFailedReason, Message: "distributed job require npu 16, instead of 8"},
		},
		{
			name:           "03 Mixed tasks - some pass, some fail - should fail on first failure",
			base910A3:      newBase910A3(16, []TaskConfig{{TaskID: "task1", ReqNPUNum: 16}, {TaskID: "task2", ReqNPUNum: 8}}),
			expectedResult: &api.ValidateResult{Pass: false, Reason: JobCheckFailedReason, Message: "distributed job require npu 16, instead of 8"},
		},
	}
