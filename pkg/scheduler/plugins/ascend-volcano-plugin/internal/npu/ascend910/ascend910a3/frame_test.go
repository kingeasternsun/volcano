...
const (
	...
	// MaxNPUNum represents the maximum number of NPUs per node for testing
	MaxNPUNum = 16
)
...
func TestBase910A3_CheckReqNPUEqualNodeNPU_ExpectError(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			base910A3:      newBase910A3(MaxNPUNum, []TaskConfig{{TaskID: TrainingWorker, ReqNPUNum: 8}}),
			expectedResult: &api.ValidateResult{Pass: false, Reason: JobCheckFailedReason, Message: fmt.Sprintf("distributed job require npu %d, instead of 8", MaxNPUNum)},
		},
		{
			name:           "03 Mixed tasks - some pass, some fail - should fail on first failure",
			base910A3:      newBase910A3(MaxNPUNum, []TaskConfig{{TaskID: TrainingWorker, ReqNPUNum: MaxNPUNum}, {TaskID: TrainingMaster, ReqNPUNum: 8}}),
			expectedResult: &api.ValidateResult{Pass: false, Reason: JobCheckFailedReason, Message: fmt.Sprintf("distributed job require npu %d, instead of 8", MaxNPUNum)},
		},
	}
